                ...
                <div class="flex gap-4 justify-start px-4 whitespace-nowrap py-2">
                    ...
                    @foreach ($categories as $category)
                        <a href="{{ route('category-wise-product', $category->slug) }}"
                            class="px-6 py-2 rounded-full font-medium transition-colors duration-300 {{ request()->route('slug') === $category->slug ? 'bg-primary text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                            {{ $category->title }}
                        </a>
                    @endforeach
                </div>
